...
Run:  python -m pytest _Tests/test_parse_csv.py -v
"""
...
class TestParseHappyPath:
    ...
    def test_access_level_values(self):
        ...
        assert rows[1]["access_level"] == "Project Administrator"


class TestParseEdgeCases:
    def test_email_trimmed_and_lowered(self):
        """Whitespace around email should be stripped, case lowered."""
        rows = parse_csv(os.path.join(FIXTURES, "FAKE_edge_cases.csv"))
...
class TestParseErrorCases:
    def test_nonexistent_project_still_parses(self):
        ...
        rows = parse_csv(os.path.join(FIXTURES, "FAKE_error_cases.csv"))
